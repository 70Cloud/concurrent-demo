...
public class ThreadSafeDemo {
    ...
    public static void main(String[] args) {
        ...
        // 线程是 CPU 调度的最小单元
        // CPU 的高速缓存，L1(L1 d 数据,L2 i 指令),L2,L3 cache，CPU 的高速缓存对其他 CPU 是不可见的，这样就导致了缓存一致性问题
        // 所以会有总线锁，缓存锁（MESI，CPU 会加 #LOCK 指令）

        // 缓存一致性协议 MESI
        // 标记位、嗅探协议
        // M(modify)：表示当前缓存被修改过
        // E(Exclusive)：独占缓存，表示该缓存只会在该 CPU 中使用，并且和主内存中的数据保持一致
        // S(Shared)：多个核心 CPU 中的数据是一致的
        // I(Invalid)：缓存失效状态
